  ...
  return (
    <div className="rounded-2xl bg-white shadow p-6 mt-8">
      <div className="flex items-center justify-between mb-4">
        ...
        <Button variant="ghost" className="text-primary text-xs font-medium flex items-center gap-1">
          View all <span>→</span>
        </Button>
      </div>
            ...
            {sampleTasks.map((task, idx) => (
              <tr key={idx} className="border-t last:border-b">
                <td className="py-2 px-2">
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${statusColor[task.status]}`}>{task.status}</span>
                </td>
